                ...
                <div className="relative rounded-lg p-5 shadow-lg max-w-md w-3/4 min-w-[300px] min-h-[200px] bg-opacity-95 bg-gray-200">
                    <div className="flex flex-col">
                        <div className="flex flex-row justify-between">
                            ...
                            <button onClick={closeModal} className="mb-4 text-sm p-2 rounded-2xl shadow-xl">Close</button>
                        </div>

                        <div className="flex flex-col w-full h-auto items-center overflow-hidden">
                            <div className="flex flex-row justify-around">
                                {[1, 2, 3, 4, 5].map((rate) => (
                                        <img key={rate} id={`rate_${rate}`} src={rate <= rates ? "/feed/icon/feed_filled_star.svg" : "/feed/icon/feed_unfilled_star.svg"} className="size-16 my-3 py-3"/>
                                ))}
                            </div>
